...
class MaskNet(nn.Module):
    def __init__(self, size=256, verbose=True):
        super().__init__()
        self.model = BiSeNet(n_classes=19)
        self.pretrained_model = 'models/pretrained_models/face_parsing_bisenet.pth'
        if verbose:
            print(f'=> Loading MaskNet from {self.pretrained_model}')
        weights = torch.load(self.pretrained_model,
    ...
    @torch.no_grad()
    def forward(self, tensor, scatter=False):
        ...
        # tensor = self.bisenet_transform(tensor[0]).unsqueeze(0)
        tensor = [self.bisenet_transform(t).unsqueeze(0) for t in tensor]
        tensor = torch.cat(tensor, dim=0)
        seg = self.model(tensor)[0]
...
def bisenet2sean(label, reverse=False):
    from misc.visualization import MASK_LABELS, ATTRS
    mask = MASK_LABELS
    attrs = {attr: i for i, attr in enumerate(ATTRS)}
    _label = torch.zeros_like(label)
    for attr in ATTRS:
        if reverse:
            _label[label == mask[attr]] = attrs[attr]
        else:
            _label[label == attrs[attr]] = mask[attr]
    return _label
