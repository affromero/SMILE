...
def get_images(images, start, end):
    s_images = images[start:end]
    if type(images) not in (list, tuple):
        return s_images
    else:
        imgs = np.array([
            np.array(Image.open(str(fn))).astype(np.float32) for fn in s_images
        ])
...
def get_activations(images,
    ...
    for i in range(n_batches):
        ...
        _images = get_images(images, start, end)
        batch = torch.from_numpy(_images).type(torch.FloatTensor)
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    ...
    if os.path.isfile(npz_path):
        ...
        m, s, act = calculate_activation_statistics(files, model, batch_size,
                                                    dims, cuda)
        np.savez(npz_path, mu=m, sigma=s, act=act)
    return m, s
