...
class LPIPS(nn.Module):
    def __init__(self):
        ...
        self.lpips_weights = self.lpips_weights
        # imagenet normalization for range [-1, 1]
        self.eval()

    def _load_lpips_weights(self):
        own_state_dict = self.state_dict()
        self.pretrained_model = 'models/pretrained_models/lpips_weights.ckpt'
        print(f'=> Loading LPIPS from {self.pretrained_model}')        
    ...
    def forward(self, x, y):
        # Imagenet normalization for -1,+1
        self.mu = torch.tensor([-0.03, -0.088, -0.188]).view(1, 3, 1, 1).to(x.device)
        self.sigma = torch.tensor([0.458, 0.448, 0.450]).view(1, 3, 1, 1).to(x.device) 
        ...
        lpips_value = 0
        for x_fmap, y_fmap, conv1x1 in zip(x_fmaps, y_fmaps,
                                           self.lpips_weights):
    ...
    @torch.no_grad()
    def calculate_lpips_given_images(self, group_of_images):
        ...
        for i in range(num_rand_outputs - 1):
            for j in range(i + 1, num_rand_outputs):
                lpips_values.append(self(group_of_images[i], group_of_images[j]))
