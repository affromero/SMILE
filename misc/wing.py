...
Lines (83 to 235) were adapted from
    ...
    def forward(self, x, heatmap=None):
        ...
        if self.with_boundary and heatmap is not None:
            ...
            xx_boundary_channel = torch.where(boundary_channel > 0.05,
                                              self.x_coords, zero_tensor).to(
                                                  zero_tensor.device)
            yy_boundary_channel = torch.where(boundary_channel > 0.05,
                                              self.y_coords, zero_tensor).to(
                                                  zero_tensor.device)
            coords = torch.cat(
                [coords, xx_boundary_channel, yy_boundary_channel], dim=1)
...
class FAN(nn.Module):
    def __init__(self,
        ...
        self.general_attr = general_attr
    ...
    def load_pretrained_weights(self, fname):
        ...
        print(f'- Loading Wings from {fname}')
    ...
    @torch.no_grad()
    def get_heatmap(self, x, b_preprocess=True, force_identity=False):
        ''' outputs 0-1 normalized heatmap '''
        x = F.interpolate(x, size=256, mode='bilinear')
        x_01 = x * 0.5 + 0.5  # Denorm
        # import ipdb; ipdb.set_trace()
        outputs, _ = self(x_01)
        heatmaps = outputs[-1][:, :-1, :, :]
        scale_factor = x.size(2) // heatmaps.size(2)
        if b_preprocess:
            _general_attr = self.general_attr and not force_identity
            heatmaps = F.interpolate(heatmaps,
                                     scale_factor=scale_factor,
                                     mode='bilinear',
                                     align_corners=True)
            heatmaps = preprocess(heatmaps, general_attr=_general_attr)
        return heatmaps
    ...
    @torch.no_grad()
    def get_visualization(self, x, current=False, dirname='.', mask=None):
        from misc.utils import denorm, save_img
        from misc.mask_utils import label2mask
        import os
        if mask is not None:
            org_x = label2mask(mask).to(x.device)
            # org_x = denorm(mask.clone())
        else:
            org_x = denorm(x.clone())
        mask = mask is None
        mode = self.general_attr
        if current:
            _general = [mode]
        else:
            _general = [mode, not mode]
        for m in _general:
            self.general_attr = m
            x0, x1 = self.get_heatmap(x.clone())
            x0 = F.interpolate(x0, size=org_x.size(-1), mode='bilinear')
            x1 = F.interpolate(x1, size=org_x.size(-1), mode='bilinear')
            x0 = (x0 * org_x).expand_as(org_x)
            x1 = (x1 * org_x).expand_as(org_x)
            list_h = torch.cat([org_x, x0, x1], dim=3)
            file_h = os.path.join(
                dirname, f'fan_general_{str(m)}_mask_{str(mask)}.jpg')
            print(f'Saving FAN visualization on: {file_h}.')
...
def tensor2numpy255(tensor):
    ...
    return ((tensor.permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5) *
            255).astype('uint8')
...
class FaceAligner():
    ...
    def align(self, imgs, output_size=256):
        ...
        for i, (img, landmarks) in enumerate(zip(imgs, landmarkss)):
            ...
            aligned = cv2.warpPerspective(img_np,
                                          transform, (cols, rows),
                                          flags=cv2.INTER_LANCZOS4)
            imgs[i] = np2tensor(
                aligned[:self.output_size, :self.output_size, :])
        return imgs
...
def pad_mirror(img, landmarks):
    ...
    small_blurred = gaussian(cv2.resize(img, (W, H)),
                             H // 100,
                             multichannel=True)
...
def align_faces(args, input_dir, output_dir):
    ...
    from misc.utils import save_img
    ...
    for fname in fnames:
        ...
        save_img(x_aligned, 1, filename=os.path.join(output_dir, fname))
...
index_map = Munch(
    ...
    remain_chin0=IDXPAIR(0, 8),
    remain_chin1=IDXPAIR(33 - 8, 33),
    eyebrows=IDXPAIR(33, 51),
    eyebrowsedges=IDXPAIR(33, 46),
    nose=IDXPAIR(51, 55),
    nostrils=IDXPAIR(55, 60),
    eyes=IDXPAIR(60, 76),
    #   centereyes=IDXPAIR(96, 97),
    centereyes=IDXPAIR(96, 98),
    #   lipedges=IDXPAIR(76, 82),
    lipedges=IDXPAIR(76, 83),
...
def preprocess(x, general_attr=False):
    ...
    if general_attr:
        # import ipdb; ipdb.set_trace()
        # x = torch.ones_like(x)
        operations = Munch(chin=OPPAIR(6 * sw, 0.1),
                           lipedges=OPPAIR(0, 0.1),
                           liplower=OPPAIR(-8 * sw, 0.01),
                           lipinner=OPPAIR(0 * sw, 0.1),
                           eyebrows=OPPAIR(0 * sw, 0.5),
                           eyes=OPPAIR(0 * sw, 1))
        # # import ipdb; ipdb.set_trace()
        for part, ops in operations.items():
            start, end = index_map[part]
            x[:, start:end] = resize(shift(x[:, start:end], ops.shift),
                                     ops.resize)

        operations = Munch(
            remain_chin0=OPPAIR(0, 1),
            remain_chin1=OPPAIR(0, 1),
            # eyebrows=OPPAIR(0 * sw, 1),
            nostrils=OPPAIR(0 * sw, 1),
            nose=OPPAIR(0 * sw, 1),
            # eyes=OPPAIR(0 * sw, 1),
            centereyes=OPPAIR(0, 1),
        )
        for part, ops in operations.items():
            start, end = index_map[part]
            x[:, start:end] = 0
    ...
    #     start, end = index_map.nose
    #     x[:, start + 1:end] = shift(x[:, start + 1:end], 4 * sw)
    #     x[:, start:end] = resize(x[:, start:end], 0.5)

    #     start, end = index_map.eyes
    #     x[:, start:end] = resize(x[:, start:end], 1)
    #     x[:, start:end] = resize(shift(x[:, start:end], -8), 0.1) + \
    #         shift(x[:, start:end], -24)

    # if general_attr:
    #     start, end = index_map.lipedges
    #     x[:, start] = resize(x[:, start], 2.5)
    #     x[:, end] = resize(x[:, end], 2.5)

    #     start, end = index_map.centereyes
    #     x[:, start] = resize(x[:, start], 2.5)
    #     x[:, end] = resize(x[:, end], 2.5)

    #     start, end = index_map.nose
    #     x[:, start:end] = resize(x[:, start:end], 1.5)

    #     start, end = index_map.eyebrows
    #     x[:, start:end] = resize(x[:, start:end], 1.5)
    # if general_attr:
    # start, end = index_map.lipedges
    # x[:, start] = resize(x[:, start], 2.5)
    ...
    # start, end = index_map.centereyes
    # x[:, start] = resize(x[:, start], 0.01)
    # x[:, end] = resize(x[:, end], 0.01)

    # # start, end = index_map.nose
    # # x[:, start:end] = resize(x[:, start:end], 0.8)

    # # start, end = index_map.eyebrows
    # # x[:, start:end] = resize(x[:, start:end], 0.01)

    # # start, end = index_map.eyebrows
    # # x[:, start:end] = resize(shift(x[:, start:end], 10*sw), 0.001)

    # start, end = index_map.chin
    # x[:, start:end] = resize(shift(x[:, start:end], -6*sw), 0.001)

    # start, end = index_map.lipedges.start, index_map.lipinner.end
    # x[:, start:end] = resize(shift(x[:, start:end], -6*sw), 0.001)

    # Second-level mask
    x2 = deepcopy(x)
    if not general_attr:
        x2[:, index_map.chin.start:index_map.chin.
           end] = 0  # start:end was 0:33
    ...
    # x2[:, index_map.chin.start:index_map.chin.
    #    end] = 0  # start:end was 0:33
    # x[:, index_map.lipupper.start:index_map.lipupper.
    #   end] = 0  # start:end was 76:81
    # x2[:, index_map.lipupper.start:index_map.lipupper.
    #    end] = 0  # start:end was 76:81
    # # x[:, index_map.eyebrows.start:index_map.eyebrows.end] = 0  #
    # # start:end was 33:51
    # x[:, index_map.liplower.start:index_map.lipinner.
    #   end] = 0  # start:end was 82:96
    # x[:, index_map.lipedges.start] = 0
    # x[:, index_map.lipedges.end] = 0
    # x2[:, index_map.lipedges.start] = 0
    # x2[:, index_map.lipedges.end] = 0
    # x2[:, index_map.liplower.start:index_map.lipinner.
    #    end] = 0  # start:end was 82:96
    # # x2[:, index_map.eyebrows.start:index_map.eyebrows.
    # #    end] = 0  # start:end was 33:51
    # # x[:, index_map.eyes.start:index_map.eyes.
    # #   end] = 0  # start:end was 60:76
    # # x2[:, index_map.eyes.start:index_map.eyes.
    # #    end] = 0  # start:end was 60:76
    # x[:, index_map.lipupper.start:index_map.lipupper.
    #    end] = 0
    # x2[:, index_map.lipupper.start:index_map.lipupper.
